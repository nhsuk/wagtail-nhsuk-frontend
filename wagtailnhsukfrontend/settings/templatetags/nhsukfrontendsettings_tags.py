...
def header(context, **kwargs):
    ...
    return {
        ...
        "primary_links": [
            {"label": link.label, "url": link.page.relative_url(site)} for link in header.navigation_links.all()
        ],
    }
...
def footer(context):
    ...
    return {
        "primary_links": [{"label": link.link_label, "url": link.link_url} for link in footer.footer_links.all()],
