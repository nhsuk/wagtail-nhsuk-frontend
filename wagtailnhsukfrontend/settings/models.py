...
class HeaderSettings(ClusterableModel, BaseSetting):
    ...
    service_link = models.ForeignKey(
        "wagtailcore.Page", null=True, blank=True, on_delete=models.SET_NULL, related_name="service_link"
    )
    ...
    logo_aria = models.CharField(
        max_length=255, null=True, blank=True, help_text="Aria label override for the NHS logo."
    )
    ...
    panels = [
        ...
        MultiFieldPanel([PageChooserPanel("logo_link"), FieldPanel("logo_aria")], heading="Logo"),
...
class NavigationLink(Orderable):
    setting = ParentalKey(HeaderSettings, on_delete=models.CASCADE, related_name="navigation_links")
    label = models.CharField(max_length=255)
    page = models.ForeignKey("wagtailcore.Page", null=True, on_delete=models.SET_NULL, related_name="+")

    panels = [FieldPanel("label"), PageChooserPanel("page")]
...
class FooterSettings(ClusterableModel, BaseSetting):
    ...
    fixed_coloumn_footer = models.BooleanField(
        default=False, help_text="Enable this setting to change way the footer is styled, so links group into coloumns"
    )
...
class FooterLinks(Orderable):
    ...
    setting = ParentalKey(FooterSettings, on_delete=models.CASCADE, related_name="footer_links")
    link_url = models.URLField(blank=True)
    link_label = models.CharField(blank=True, max_length=250)

    panels = [FieldPanel("link_url"), FieldPanel("link_label")]
