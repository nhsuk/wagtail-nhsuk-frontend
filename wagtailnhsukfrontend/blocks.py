    ...
    def clean(self, value):
        ...
        if not url_links:
            errors["internal_page"] = ErrorList(
                ["Please choose a page or enter a URL above."]
            )
            errors["external_url"] = ErrorList(
                ["Please enter a URL or choose a page below."]
            )
        elif url_links > 1:
            errors["internal_page"] = ErrorList(
                ["Please only enter a URL or choose a page."]
            )
            errors["external_url"] = ErrorList(
                ["Please only enter a URL or choose a page."]
            )
...
class ImageBlock(FlattenValueContext, StructBlock):
    ...
    alt_text = CharBlock(
        required=False, help_text="Only leave this blank if the image is decorative."
    )
...
class CardClickableBlock(CardBasicBlock):
    ...
    def clean(self, value):
        ...
        if not url_links:
            errors["internal_page"] = ErrorList(
                ["Please choose a page or enter a URL below."]
            )
        ...
        if errors:
            raise ValidationError(
                "Validation error in CardClickableBlock", params=errors
            )
...
class CareCardBlock(FlattenValueContext, StructBlock):
    ...
    heading_level = IntegerBlock(
        ...
        min_value=2,
        max_value=6,
        default=CARE_CARD_HEADING_LEVEL,
        help_text="The heading level affects users with screen readers. Default="
        + str(CARE_CARD_HEADING_LEVEL)
        + ", Min=2, Max=4.",
    )
    title = CharBlock(required=True)
